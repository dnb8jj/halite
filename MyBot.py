...
while True:
    ...
    if check_area(hotspot) < 1000 and hstp_search_cnt < 20:
        hstp_search_cnt += 1
    ...
    logging.info("Movement Phase: turn " + str(turn) + ", the hotspot is " + format(hotspot))
    # command ships
    for ship in me.get_ships():
        ...
        if stay == 0 and not game_map[ship.position.directional_offset(move_dir)].is_occupied:
            ...
            logging.info(
                "ship @ " + format(ship.position) + " move to " + format(ship.position.directional_offset(rand)))
        else:
            command_queue.append(ship.stay_still())
            game_map[ship.position].mark_unsafe(ship)
            logging.info("ship @ " + format(ship.position) + "stayed still and collected {}.".format(
                game_map[ship.position].halite_amount / 4))
